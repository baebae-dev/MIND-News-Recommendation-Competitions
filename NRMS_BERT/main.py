...
from utils.utils import parallelize_eval, evaluation
...
def main(gpus, local_rank, data_path, data, out_path, config_path, eval_every):
    ...
    for epoch in range(1, epochs+1):
        ...
        evaluation
        '''
        # vlds = {'his': vld_his, 'pop': vld_pop, 'fresh': vld_fresh, 'impr_idx': vld_impr_idx,
        #         'impr': vld_impr, 'label': vld_label}
        # metrics2 = {metric: 0. for metric in config['metrics']}
        # eval_st0 = time.time()
        # metrics2 = parallelize_eval(list(range(len(vld_impr))), evaluation, metrics2,
        #                             vlds, config, DEVICE, model, num_cores=config['num_cores'])
        # print("Parallel eval time: ", time.time() - eval_st0)
        ...
        for metric, _ in metrics.items():
            # print(metric, ":",metrics[metric], metrics2[metric], metrics[metric]==metrics2[metric])
