...
# File: src/NRMS_BERT/utils/dataloader.py
...
from utils.utils_test import parallelize_dataframe_test, process_behaviors_test
...
class DataSet(torch.utils.data.Dataset):
    ...
    def init_news(self, news_file):
        ...
        Outputs:
            ...
            title_embeddings: embeddings for the titles
            news_cat_embeddings: embeddings for categories
            news_subcat_embeddings: embeddings for subcategories
            abs_embeddings: embedding for abstracts
        """
    ...
    def init_behaviors(self, behaviors_file):
        ...
        Outputs:
            ...
            raw_impr_indexes: indices for the behvaiod items
            impr_indexes: index of the behavior
            uindexes: index of users
            pops: indices of the popular news
            freshs: indices of the fresh news
        """
        if 'dev' in behaviors_file or 'train' in behaviors_file:
            _dev_test = 'dev'
        elif 'test' in behaviors_file:
            _dev_test = 'test'
        

        st = timeit.default_timer()
        if _dev_test == 'dev':
            df = pd.read_csv(behaviors_file, sep='\t', header=None, quoting=csv.QUOTE_NONE)
            df = parallelize_dataframe(df, process_behaviors, num_cores=self.num_cores,
                                       class_pointer=self)
        elif _dev_test == 'test':
            df = pd.read_csv(behaviors_file, sep='\t', header=None, quoting=csv.QUOTE_NONE)
            df = parallelize_dataframe_test(df, process_behaviors_test, num_cores=self.num_cores,
                                       class_pointer=self)
...
class DataSetTrn(DataSet):
    ...
    def __getitem__(self, idx):
        ...
        his = {'title': self.title_embeddings[self.histories_unfold[idx]],
               ...
               'abstract': self.abs_embeddings[self.histories_unfold[idx]]}
        ...
        pop = {'title': self.title_embeddings[self.pop_unfold[idx]],
               ...
               'abstract': self.abs_embeddings[self.pop_unfold[idx]]}
        ...
        fresh = {'title': self.title_embeddings[self.fresh_unfold[idx]],
                 ...
                 'abstract': self.abs_embeddings[self.fresh_unfold[idx]]}
...
class DataSetTest(DataSet):
    ...
    def __getitem__(self, idx):
        ...
        impr = {'title': self.title_embeddings[self.imprs_unfold[idx]],
                ...
                'abstract': self.abs_embeddings[self.imprs_unfold[idx]]}
