        ...
        if local_rank==0:
            with open(os.path.join(out_path, f'prediction-{epoch}.txt'), 'w') as f:
                for j in tqdm(range(len(vld_impr)), desc='Evaluation', total=len(vld_impr)):
                    ...
                    vld_his_j = (vld_his[j]).to(DEVICE).unsqueeze(0)
                    vld_pop_j = (vld_pop[j]).to(DEVICE).unsqueeze(0)
                    vld_fresh_j = (vld_fresh[j]).to(DEVICE).unsqueeze(0)
                    ...
                    vld_cand_j = (vld_impr[j]).to(DEVICE).unsqueeze(0)
