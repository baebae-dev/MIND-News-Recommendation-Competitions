...
from utils.selector_test import NewsSelectorTest
...
def main(data_path, data, out_path, config_path, eval_every):
    ...
    out_path = os.path.join(out_path, f'MIND{data}_dev_NRMS_Aux_pop{num_pop}_fresh{num_fresh}')
    ...
    for epoch in range(1, epochs+1):
        ...
        with open(os.path.join(out_path, f'test-prediction-{epoch}.txt'), 'w') as f:
            for j in tqdm(range(len(vld_impr_title)), desc=f'Epoch {epoch:3d} test evaluation', total=len(test_impr_title)):
                impr_idx_j = test_impr_idx[j]
                test_his_title_j = torch.tensor(test_his_title[j]).long().to(DEVICE).unsqueeze(0)
                test_pop_title_j = torch.tensor(test_pop_title[j]).long().to(DEVICE).unsqueeze(0)
                test_fresh_title_j = torch.tensor(test_fresh_title[j]).long().to(DEVICE).unsqueeze(0)
                test_pop_title_j = test_pop_title_j[:, :config['pop'], :]
                test_fresh_title_j = test_fresh_title_j[:, :config['fresh'], :]
                test_cand_title_j = torch.tensor(test_impr_title[j]).long().to(DEVICE).unsqueeze(0)
                test_global_title_j = torch.cat((test_pop_title_j, test_fresh_title_j), dim=1)

                if config['aux']:
                    test_his_abs_j = torch.tensor(test_his_abs[j]).long().to(DEVICE).unsqueeze(0)
                    test_pop_abs_j = torch.tensor(test_pop_abs[j]).long().to(DEVICE).unsqueeze(0)
                    test_fresh_abs_j = torch.tensor(test_fresh_abs[j]).long().to(DEVICE).unsqueeze(0)
                    test_pop_abs_j = test_pop_abs_j[:, :config['pop'], :]
                    test_fresh_abs_j = test_fresh_abs_j[:, :config['fresh'], :]
                    test_cand_abs_j = torch.tensor(test_impr_abs[j]).long().to(DEVICE).unsqueeze(0)
                    test_global_abs_j = torch.cat((test_pop_abs_j, test_fresh_abs_j), dim=1)

                    test_his_cat_j = torch.tensor(test_his_cat[j]).long().to(DEVICE).unsqueeze(0)
                    test_pop_cat_j = torch.tensor(test_pop_cat[j]).long().to(DEVICE).unsqueeze(0)
                    test_fresh_cat_j = torch.tensor(test_fresh_cat[j]).long().to(DEVICE).unsqueeze(0)
                    test_pop_cat_j = test_pop_cat_j[:, :config['pop']]
                    test_fresh_cat_j = test_fresh_cat_j[:, :config['fresh']]
                    test_cand_cat_j = torch.tensor(test_impr_cat[j]).long().to(DEVICE).unsqueeze(0)
                    test_global_cat_j = torch.cat((test_pop_cat_j, test_fresh_cat_j), dim=1)

                    test_his_subcat_j = torch.tensor(test_his_subcat[j]).long().to(DEVICE).unsqueeze(0)
                    test_pop_subcat_j = torch.tensor(test_pop_subcat[j]).long().to(DEVICE).unsqueeze(0)
                    test_fresh_subcat_j = torch.tensor(test_fresh_subcat[j]).long().to(DEVICE).unsqueeze(0)
                    test_pop_subcat_j = test_pop_subcat_j[:, :config['pop']]
                    test_fresh_subcat_j = test_fresh_subcat_j[:, :config['fresh']]
                    test_cand_subcat_j = torch.tensor(test_impr_subcat[j]).long().to(DEVICE).unsqueeze(0)
                    test_global_subcat_j = torch.cat((test_pop_subcat_j, test_fresh_subcat_j), dim=1)

                    test_his_j = [test_his_title_j, test_his_abs_j, test_his_cat_j, test_his_subcat_j]
                    test_cand_j = [test_cand_title_j, test_cand_abs_j, test_cand_cat_j, test_cand_subcat_j]
                    test_global_j = [test_global_title_j, test_global_abs_j, test_global_cat_j, test_global_subcat_j]
                else:
                ...
                if config['global']:
                    test_user_out_j = model((test_his_j, test_global_j), source='pgt')
                else:
                    test_user_out_j = model(test_his_j, source='history')

                test_cand_out_j = model(test_cand_j, source='candidate')

                scores_j = torch.matmul(test_cand_out_j, test_user_out_j.unsqueeze(2)).squeeze()
                scores_j = scores_j.detach().cpu().numpy()
                argmax_idx = (-scores_j).argsort()
                ranks = np.empty_like(argmax_idx)
                ranks[argmax_idx] = np.arange(1, scores_j.shape[0]+1)
