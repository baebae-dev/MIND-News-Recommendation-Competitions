...
# File: src/NRMS_NAML/utils/selector_test.py
# - This file includes utility functions useful for selecting popular/fresh news
#
# Version: 1.0
#######################################################################################################
...
class NewsSelectorTest(object):
    ...
    def __init__(self, data_type1, data_type2, num_pop, num_fresh, bucket_size=3):
        ...
        :param data_type1: data type (large, demo)
        :param data_type2: data type (train, dev, test)
        :param num_pop: number of popular news
        :param num_fresh: number of fresh news
    ...
    def get_fresh(self, time):
        ...
        :return: fresh news at query time
        """
    ...
    def get_pop_clicked(self, time):
        """
        Get popular news by clicked information (only -1 news).
        :param time: query time
        :return: list of the popular news
        """
    ...
    def get_pop_recommended(self, time):
        """
        Get popular news by recommended information (use both -0 and -1 news).
        :param time: query time
        :return: list of the popular news
        """
    ...
    def load_pop_hash_clicked(self):
        """
        Load popular clicked news hash.
        """
    ...
    def load_pop_hash_recommended(self):
        """
        Load popular recommended news hash.
        """
    ...
    def generate_fresh_news_df(self):
        ...
        for i in tqdm.tqdm(range(df.shape[0])):
            _imp = df.iloc[i, :]
            ltime = self._process_news_date(_imp[2])
            imprs = _imp[4].split(' ')
            for imp in imprs:
                if imp in news2time:
    ...
    def generate_pop_hash_clicked(self):
        """
        Generate popular clicked news hash.
        """
        df = pd.read_csv(self.behavior_file, sep='\t', header=None)
        buckets = {}
    ...
    def generate_pop_hash_recommended(self):
        """
        Generate popular recommended hash.
        """
        df = pd.read_csv(self.behavior_file, sep='\t', header=None)
        buckets = {}
        for i in range(df.shape[0]):
    ...
    def parsing_behavior_time(self, ftime, return_type='list'):
        """
        Parse bahavior time.
        :param ftime: time string
        :param return_type: data type
        :return: parsed time
        """
    ...
    def get_news_list(self, raw_imp, all=False):
        """
        Get news list.
        :param raw_imp: raw impression string
        :param all: whether to return all impression
        :return: news list
        """
        _imps = raw_imp.split(' ')
        if all:
