    ...
    for lt in truth_f:
        ...
        if ls == '':
            ...
            try:
                ...
                raise ValueError(
                    "line-{}: Invalid Input Format!".format(line_index))

        if sub_impid != impid:
            raise ValueError(
                "line-{}: Inconsistent Impression Id {} and {}".format(
                    line_index,
                    sub_impid,
                    impid
                ))
        ...
        for rank in sub_ranks:
            ...
            if score_rslt < 0 or score_rslt > 1:
                raise ValueError(
                    "Line-{}: score_rslt should be int from 0 to {}".format(
                        line_index,
                        lt_len
                    ))
...
if __name__ == '__main__':
    ...
    if os.path.isdir(submit_dir) and os.path.isdir(truth_dir):
        ...
        submission_answer_file = open(
            os.path.join(submit_dir, "prediction.txt"), 'r')

        auc, mrr, ndcg, ndcg10 = scoring(truth_file, submission_answer_file)

        output_file.write(
            "AUC:{:.4f}\nMRR:{:.4f}\nnDCG@5:{:.4f}\nnDCG@10:{:.4f}".format(auc,
                                                                           mrr,
                                                                           ndcg,
                                                                           ndcg10))
        output_file.close()